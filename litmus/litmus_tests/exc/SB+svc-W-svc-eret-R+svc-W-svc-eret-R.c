...
static void P0(litmus_test_run* data) {
  asm volatile(
    ...
    : "cc", "memory", "x0", "x1", "x2", "x3", "x7", "x10", "x11"
  );
}
...
static void P1(litmus_test_run* data) {
  asm volatile(
    ...
    :
    : ASM_VARS(data, VARS), ASM_REGS(data, REGS)
    : "cc", "memory", "x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x10", "x11" /* dont touch parameter registers */
  );
}

litmus_test_t SB_svcWsvceretR_svcWsvceretR = {
  "SB+svc-W-svc-eret-R+svc-W-svc-eret-R",
  MAKE_THREADS(2),
  MAKE_VARS(VARS),
  MAKE_REGS(REGS),
