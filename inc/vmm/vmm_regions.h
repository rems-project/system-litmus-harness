 ...
 */
#define STACK_SAFE_PA(va) (!IN_STACK_MMAP_SPACE(va) ? va : STACK_MMAP_VA_TO_PA(va))
...
#define THR_VTABLE_VA(t) (ENABLE_PGTABLE ? (u64*)(VTABLE_MMAP_BASE + PAGE_SIZE * t) : THR_VTABLE_PA(t))
 ...
 */
#define HARNESS_MMAP_PHYS_TO_VIRT(p) (HARNESS_MMAP_BASE + __HARNESS_MMAP_OFFSET(p))
...
#define SAFE_TESTDATA_VA(va) \
  ((!IN_TESTDATA_RANGE(va)) ? HARNESS_MMAP_PHYS_TO_VIRT(va) : HARNESS_MMAP_PHYS_TO_VIRT(TESTDATA_MMAP_VIRT_TO_PHYS(va)))

#define SAFE_TESTDATA_PA(va) ((!IN_TESTDATA_RANGE(va)) ? va : TESTDATA_MMAP_VIRT_TO_PHYS(va))
