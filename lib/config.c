  ...
  .args =
    (const argdef_arg_t*[]){
      OPT(
        ...
        .show_help_both = 1, .arg = OPT_ARG_OPTIONAL, .metavar = "OPTION",
      ),
      FLAG_ACTION(
        "-V", "--version", version,
      ...
      FLAG_ACTION(
        NULL, "--id", device_ident_and_quit,
        "display device identification information and quit\n"
        "\n"
        "shows the information for the current device, and all ID registers.",
        .no_negation = true,
      ),
      FLAG("-p", "--pgtable", ENABLE_PGTABLE, "enable/disable pagetable\n"),
      FLAG("-t", "--trace", TRACE, "enable/disable tracing\n"),
      FLAG("-d", "--debug", DEBUG, "enable/disable debugging\n"),
      FLAG(NULL, "--verbose", VERBOSE, "enable/disable verbose output\n"),
      OPT(
        "-q", "--quiet", q,
        "quiet mode\n"
        "\n"
...
argdef_t LITMUS_ARGS = (argdef_t){
  ...
  .args =
    (const argdef_arg_t*[]){
      ...
      FLAG(NULL, "--run-forever", RUN_FOREVER, "repeat test runs indefinitely\n"),
      FLAG(NULL, "--perf", ENABLE_PERF_COUNTS, "enable/disable performance tests\n"),
      OPT(
        "-n", NULL, n,
      ...
      FLAG(NULL, "--hist", ENABLE_RESULTS_HIST, "enable/disable results histogram collection\n"),
      FLAG(
        NULL, "--print-outcome-breakdown", ENABLE_RESULTS_OUTREG_PRINT,
        "prints the breakdown of observed outcomes (default: on)\n"
      ),
      FLAG(
        NULL, "--print-missing-warning", ENABLE_RESULTS_MISSING_SC_WARNING,
        "prints a warning if an expected outcome is not observed (default: on)\n"
      ),
      ENUMERATE(
        "--tlbsync", LITMUS_SYNC_TYPE, sync_type_t, 4, ARR((const char*[]){ "none", "asid", "va", "all" }),
      ...
      ENUMERATE(
        "--aff", LITMUS_AFF_TYPE, aff_type_t, 2, ARR((const char*[]){ "none", "rand" }),
      ...
      ENUMERATE(
        "--shuffle", LITMUS_SHUFFLE_TYPE, shuffle_type_t, 2, ARR((const char*[]){ "none", "rand" }),
      ...
      ENUMERATE(
        "--concretize", LITMUS_CONCRETIZATION_TYPE, concretize_type_t, 3,
      ...
      OPT(
        NULL, "--config-concretize", conc_cfg,
...
argdef_t UNITTEST_ARGS = (argdef_t){
  ...
  .args =
    (const argdef_arg_t*[]){
      FLAG(
        NULL, "--test-linear-concretization", ENABLE_UNITTESTS_CONCRETIZATION_TEST_LINEAR,
        "include linear concretization tests (default: on)\n"
      ),
      FLAG(
        NULL, "--test-random-concretization", ENABLE_UNITTESTS_CONCRETIZATION_TEST_RANDOM,
