...
void run_test(const char* name, int no_threads, th_f** funcs, int no_heap_vars,
    ...
    trace("%s\n", "Running Tests ...");
    for (int i = 3; i >= 0; i--) {
        run_on_cpu(i, (async_fn_t*)go_cpus, (void*)&ctx);
    }
...
static void run_thread(test_ctx_t* ctx, int cpu) {
    ...
    for (int j = 0; j < ctx->no_runs; j++) {
        ...
        func(ctx, i, (uint64_t**)heaps, (uint64_t**)ptes, (uint64_t*)pas, (uint64_t**)regs);
        end_of_run(ctx, cpu, i);
        _check_ptes(ctx, ctx->no_heap_vars, heaps, ptes, saved_ptes);
        bwait(cpu, i % ctx->no_threads, &ctx->cleanup_barriers[i],
              ctx->no_threads);
    }
}
...
void init_test_ctx(test_ctx_t* ctx, const char* test_name, int no_threads,
    ...
    *final_barrier = 0;
    ...
    test_hist_t* hist =
        alloc(sizeof(test_hist_t) + sizeof(test_result_t) * 200);
    hist->allocated = 0;
    hist->limit = 200;
...
void start_of_run(test_ctx_t* ctx, int thread, int i) {
    ...
    /* prefetch(ctx, i); */
    bwait(thread, i % ctx->no_threads, &ctx->start_barriers[i],
          ctx->no_threads);
...
void print_results(test_hist_t* res, test_ctx_t* ctx, const char** out_reg_names,
                   uint64_t* interesting_results) {
    printf("\n");
    printf("Test %s:\n", ctx->test_name);
    int marked = 0;
