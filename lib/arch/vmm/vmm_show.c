...
void __vmm_dump_node(u64* parent_table, int level, int index, u64* entry, desc_t desc, int is_leaf, va_range range, void* data) {
  if (! is_leaf)
...
void __vmm_count_tables(u64* parent_table, int level, int index, u64* entry, desc_t desc, int is_leaf, va_range range, void* data) {
  u64* ctr = data;
  if (! is_leaf)
    *ctr += 1;
}
