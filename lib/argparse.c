...
int argdef_try_split(
  char* lhs, char* rhs, char* argv, char* short_name, char* long_name, argdef_option_action_kind_t opt_arg
) {
...
void argparse_read_args(int argc, char** argv) {
  for (int i = 0; i < argc; i++) {
    switch (*argv[i]) {
    case '-':
      if (!argparse_read_arg(THIS_ARGS, argv[i]) && !argparse_read_arg(&COMMON_ARGS, argv[i]))
